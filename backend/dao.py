...
class UserDao:
    ...
    def get_authcode(self, authcode:int) -> Authcode:
        data = self.db_session.query(Authcode).filter_by(authcode=authcode).one_or_none()
        self.db_session.close()
    ...
    def get_user(self, uid:int=None, email:str=None) -> User:
        if uid == None and email == None:
            ...
            data = self.db_session.query(User).filter_by(uid=uid).one_or_none()
            self.db_session.close()
            return data
        elif email != None: 
            data = self.db_session.query(User).filter_by(email=email).one_or_none()
            self.db_session.close()
            return data
